        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: ElevatedButton.icon(style: btnStyle(context, Theme.of(context).primaryColor), onPressed: (){
                Navigator.of(context).push(MaterialPageRoute(builder: (context) =>RegisterEmailPageSend()));
              }, icon: Icon(Icons.email_rounded, size: 64,), label: Text("signupEmail".tr().toString(), textAlign: TextAlign.center, style: TextStyle(fontSize: 24))),
            ),
            SizedBox(height: 50,),
            Container(
              width: MediaQuery.of(context).size.width-50,
              child: ElevatedButton.icon(style: btnStyle(context, Colors.white), onPressed: ()async{
                var result;
                ...
                String read = await result;
                  if(read == 'Signed up'){
                    FirebaseFirestore.instance.collection('Users').doc(googleAccount!.email).set({'email': googleAccount!.email, 'name': googleAccount!.displayName, 'password': googleAccount!.id, 'credit': 0, 'status': 0, 'xp': 0, 'language': 'tr', 'method': 'google', 'settings': initialSettings});
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(new SnackBar(content: Text(read)));
              }, icon: Image.asset("assets/imgs/google.png", width: 64,), label: Text("signupGoogle".tr().toString(), textAlign: TextAlign.center, style: TextStyle(fontSize: 24, color: Colors.black))),
            )]
        ),
      ),
    );
...
class LoginPage extends State<LoginPageSend>{
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("login".tr().toString()), centerTitle: true,),
      body: 
      Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Container(
              width: MediaQuery.of(context).size.width-50,
              child: ElevatedButton.icon(style: btnStyle(context, Theme.of(context).primaryColor), onPressed: (){ 
                Navigator.of(context).push(MaterialPageRoute(builder: (context) =>LoginEmailPageSend()));
              }, icon: Icon(Icons.email_rounded, size: 64,), label: Text("loginEmail".tr().toString(), style: TextStyle(fontSize: 24))),
            ),
          SizedBox(height: 50,),
          Container(
            width: MediaQuery.of(context).size.width-50,
            child: ElevatedButton.icon(style: btnStyle(context, Colors.white),onPressed: ()async{ 
              ...
              if(read == 'Signed in'){
                ...
                ScaffoldMessenger.of(context).showSnackBar(new SnackBar(content: Text(read)));
            }, icon: Image.asset("assets/imgs/google.png", width: 64,), label: Text("loginGoogle".tr().toString(), style: TextStyle(fontSize: 24, color: Colors.black))),
          ),
        ],
        ),
      )
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              }, icon: Icon(Icons.login, size: 64,), label: Text("login".tr().toString(), style: TextStyle(fontSize: 24))),
            ),
            SizedBox(height: 25,),
            Container(
              width: MediaQuery.of(context).size.width-50,
              ...
              }, icon: Icon(Icons.person_add, size: 64,), label: Text("signup".tr().toString(), textAlign: TextAlign.center, style: TextStyle(fontSize: 24))),
            ),
          ],
        )
      ),
...
class RegisterEmailPageSend extends StatefulWidget{
  RegisterEmailPageSend();
  @override
  State<StatefulWidget> createState() {
    return RegisterEmailPage();
  }
}
class RegisterEmailPage extends State<RegisterEmailPageSend>{
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text("signup".tr().toString(), textAlign: TextAlign.center,), centerTitle: true, 
        actions: [
          Container(
            ...
            ElevatedButton(onPressed: () async{
              ...
              if(email.text != '' && name.text != '' && password.text != ''){
                ...
                if(result == 'Signed up'){
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(new SnackBar(content: Text(result)));
                }
              }
              else{
...
class LoginEmailPageSend extends StatefulWidget{
  LoginEmailPageSend();
  @override
  State<StatefulWidget> createState() {
...
class LoginEmailPage extends State<LoginEmailPageSend>{
  TextEditingController email = new TextEditingController();
  TextEditingController password = new TextEditingController();
  @override
  Widget build(BuildContext context) {
          ...
          actions: [
            ElevatedButton.icon(onPressed: (){
              ...
              return SingleChildScrollView(
                ...
                child: AlertDialog(
                  title: Text('help'.tr().toString(), textAlign: TextAlign.center,),
                  content: RichText(text: TextSpan(
                    style: TextStyle(fontSize: 18.0, color: Colors.black),
                    children: [
                      TextSpan(text: 'helpInfoA'.tr().toString()),
                      TextSpan(text: 'helpInfoB'.tr().toString(), style: TextStyle(fontWeight: FontWeight.bold)),
                      TextSpan(text: 'helpInfoC'.tr().toString(), ),
                      TextSpan(text: 'helpInfoD'.tr().toString(), style: TextStyle(fontWeight: FontWeight.bold)),
                      TextSpan(text: 'helpInfoE'.tr().toString()),
                      TextSpan(text: 'apply'.tr().toString(), style: TextStyle(fontWeight: FontWeight.bold)),
                      TextSpan(text: 'helpInfoF'.tr().toString()),
                    ]
                  ),)
                ),
              );
          ...
          Center(
            ...
            ElevatedButton(onPressed: () async{
              if(this.user?.method == 'email'){
                ...
                FirebaseFirestore.instance.collection('Users').doc(email.text).set({'email': this.user?.email, 'name': this.user?.name, 'password': this.user?.password, 'status': 0, 'xp': this.user?.xp, 'credit': 0, 'language': this.user?.language, 'method':this.user?.method, 'settings': this.user?.settings}); 
              }
      ...
      child: Scaffold(
        ...
        body: Column(
          ...
          children: [
            ...
            ListTile(
              leading: Text('digit'.tr().toString(), style: TextStyle(fontSize: 24),),
              trailing: DropdownButton<int>(
    ...
    return WillPopScope(
        onWillPop: (() => onWillPop(context)),
          child: Scaffold(
            appBar: AppBar(
              centerTitle: true,
              title: Text(roundCounter==0?'Sayi Bulmaca':'Round ' + roundCounter.toString() + "/" + this.options!.bestOf.toString()),
              actions: [
